...
public class Population {
     ...
     */
    public static Population createInitialPopulation(int populationSize, int numberOfGenes, Random random) {
        Population population = new Population(populationSize);

        for (int i = 0; i < populationSize; i++) {
            population.setChromosome(i, generateChromosome(numberOfGenes, random));
        }
     ...
     *
     * @param numberOfGenes Number of genes in a chromosome.
     * @param random        Random singleton used for random number generation.
     * @return Randomly generated chromosome.
     */
    private static Chromosome generateChromosome(int numberOfGenes, Random random) {
        Chromosome chromosome = new Chromosome(numberOfGenes);

        for (int i = 0; i < numberOfGenes; i++) {
            chromosome.setGene(i, 1 + 0.5 * random.nextGaussian());
        }
